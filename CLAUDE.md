...
- **Cross-viewport / browser-class audit (V1_464):** 39-cell matrix (12 desktop sizes 1024→3440 incl. 5:4 + short windows, mac-Safari / Firefox / Edge UA emulation, 6 phones portrait+landscape, 6 iPad orientations, `prefers-reduced-motion`) screenshotted and geometry-checked against the pre-redesign build. Phone + iPad layouts verified **byte-identical** to V1_458 except the three intended changes (badge chip moved to the cover bottom, `.coming-soon-sub` added, Pinball pill removed). One fix: the desktop hero `#home h1` now releases the base `.home-section h1` `white-space: nowrap` / `flex-shrink: 0` inside the ≥1100px fence. The `<br>` already forces the 2-line lockup, so nowrap only set the failure mode — the title renders in the platform `cursive` fallback (Apple Chancery on macOS, Comic Sans on Windows), whose advance widths vary by more than the ~1.26x width headroom the left column has, and an over-wide line slid under the opaque album art and silently hid its last letters. It now wraps inside the column instead; h1 geometry is unchanged in every matrix cell on a fallback that fits. Ultrawide (≥2400px) keeps the thinnest headroom (~1.15x) — capping `font-size` at 8.6rem instead of 9.5rem would even that out if Kevin wants it.
- **Merch wired to the Fourthwall launch lineup (V1_467):** the five placeholder cards are replaced by the launch-day SKUs from `docs/MERCH_PLAN_fourthwall.md` (Classic Tee $25 / Heavyweight Tee $32 / Pullover Hoodie $49 / Dad Hat $30 / Die-Cut Sticker $7, USD as Fourthwall displays), each with a 600x600 WebP tile in `Images/Merch/` — **placeholders generated from `merch/artwork/`, to be swapped for the real Fourthwall mockups**. Links are built from ONE place: `FOURTHWALL_STORE` + `FOURTHWALL_PRODUCTS` in the head script, keyed off `data-product` slugs. Empty store constant = not live: COMING SOON + `.coming-soon-sub` stay up, cards keep `href="#"` + `aria-disabled` and a click-blocker (the dead `PAYMENT_LINK_` paths are gone). Set the constant and the script adds `html.merch-live`, which reveals the h2 + grid and hides the banner exactly like `html.beta-pay` (still the `?beta=pay` preview flag) — every gate rule now carries both selectors. Desktop ≥1100px fence: one row of 5, square tiles, Poppins names, gold `#ffd700` prices, `.home-cta`-style ghost pill. Fixes found while auditing the previously-unseen grid: the reveal switched from `display:grid` to `display:flex` (the grid collapsed the cards into one stretched column and clipped `.merch-info` on desktop); 1000–1099px desktop trims the card so all five stay on one row; phone/tablet cards size to their content (the fixed `aspect-ratio: 1/1.15` clipped the CTA pill) with the tile capped at 96px, 72px on landscape tablets, 64px + a 3-across bounded-scroll grid on short landscape phones. Gated default verified pixel-identical to V1_464 at 1920x1080, 412x915 and 768x1024.



**V1_468 (cross-browser background/effects parity):**

- **Target look = Firefox.** The background wallpaper + effect layers now render from ONE shared set of rules in all three engines instead of three per-browser forks. Measured with a corrected GitHub-Actions probe (Playwright chromium/firefox/webkit at 1920x1080; probe v1's tables were void because headless Firefox missed the `(hover:hover) and (pointer:fine)` desktop fence and the CSS animation freeze landed each engine on a different keyframe — v2 launches Firefox with `ui.*PointerCapabilities: 6`, freezes through the Web Animations API and asserts both).
- **What was actually diverging was CSS, not the engines.** Forced-class passes showed the three engines rendering identical declarations to within ~1%. The differences came from: (a) `@supports (-webkit-appearance:none) and (not (-moz-appearance:none))` — a Chromium+WebKit-only block that hid `.liquid-overlay`/`.psych-fractals`, forced `.oil-slick` from `color-dodge` to `screen`, and switched ON `.chrome-color-sweep` + `.chrome-shimmer` (two extra magenta/green washes at opacity 0.9/0.85 that Firefox never painted); (b) `@media screen and (-webkit-min-device-pixel-ratio:0)` — also Chromium+WebKit — replacing the 130% wallpaper oversize and 10-stop mask with a 110% box and a hard 8%/92% mask; (c) Safari hiding `section::before` entirely and painting the wallpaper on the section (no oversize, no mask fade) from **three** places at once, one of them inline `!important` from JS; (d) a Safari-only `section::after` colour wash; (e) three sets of saturation/opacity numbers.
